...
def main(spark):
    ...
    df_small_repartitioned = df_small.repartition(col("first_name"), col("loyalty"))
    df_moderate_repartitioned = df_moderate.repartition(col("first_name"), col("loyalty"))
    df_big_repartitioned = df_big.repartition(col("first_name"), col("loyalty"))

    # Convert to Parquet and save to HDFS directory
    df_small_repartitioned.write.mode('overwrite').parquet('hdfs:/user/qy561_nyu_edu/peopleSmallOpt3-2Brian.parquet')
    df_moderate_repartitioned.write.mode('overwrite').parquet('hdfs:/user/qy561_nyu_edu/peopleModerateOpt3-2Brian.parquet')
    df_big_repartitioned.write.mode('overwrite').parquet('hdfs:/user/qy561_nyu_edu/peopleBigOpt3-2Brian.parquet')
